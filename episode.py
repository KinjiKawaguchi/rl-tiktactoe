...
eta = 0.1  # 学習率
episodes = np.arange(1000,100000,1000)
gamma = 0.9  # 時間割引率
initial_epsilon = 0.5  # ε
episode = 100  # エピソード数

def worker(set_index, return_dict,episode):
    winner_list = []
    # Q学習テーブルを初期化
...
if __name__ == "__main__":
    print(f"START: {datetime.datetime.now()}")  # 現在時刻を出力

    # 学習セット数
    learning_sets = 100
    start_time = time.time()  # 開始時間の記録
    mean_win_rates = []  # 各etaの勝率の平均を保存するリスト
    ...
    for i,episode in enumerate(episodes):
        ...
        for set_index in range(learning_sets):
            p = mp.Process(target=worker, args=(set_index, return_dict,episode))
            p.start()
            processes.append(p)
        ...
        mean_win_rates.append(mean_win_rate)  # etaの勝率の平均をリストに追加
        ...
        print(f"Completion: {episode}done")  # 完了率を出力
        print(f"Current time: {datetime.datetime.now()}")  # 現在時刻を出力
        ...
        # PNGとして保存
        plt.savefig(f"win_rate_distribution_episode{episode}_mean{mean_win_rate}_median{median_win_rate}_variance{variance}_std_deviation{std_deviation}.png")

        plt.clf()  # 描画をリセット

    # 全てのetaが処理された後、x軸に学習率、y軸に平均勝率をプロットしたグラフを作成
    plt.plot(episodes, mean_win_rates)
    plt.xlabel('Learning Rate (episode)')
    plt.ylabel('Mean Win Rate')
    plt.title('Mean Win Rate vs Learning Rate')

    # PNGとして保存
    plt.savefig('mean_win_rate_vs_learning_rate.png')
